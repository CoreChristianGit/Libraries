/*  Name: myFirstLibraryWithInterrupt
 *  Date Created: 23/05/2018
 *  Description: Demonstrates the use of the myFirstLibrary library methods using interrupts
 *  Connections:
...
float potValue = 0;
int brightness = 255;
const int mode = FALLING;       //  LOW = constant, FALLING = initial
...
void setup(){
  ...
  attachInterrupt(digitalPinToInterrupt(butPin), buttonPress_ISR, mode);
  Serial.begin(9600);
}
...
void buttonPress_ISR(){
  Serial.println("We are now in the interrupt Service Routine");
